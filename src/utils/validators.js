export const validators = {
  ...
  expiryMonth: (value) => /^(0[1-9]|1[0-2])$/.test(value) || "Expiry month must be between 01 and 12.",
  expiryYear: (value) => /^\d{2}$/.test(value) || "Expiry year must be 2 digits.",
  cvv: (value) => /^\d{3,4}$/.test(value) || "CVV must be 3 or 4 digits.",
  cardHolderName: (value) => /^[a-zA-Z\s]+$/.test(value) || "Cardholder name must contain only letters and spaces.",
